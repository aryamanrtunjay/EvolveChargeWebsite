...
export default function EngineeringDashboard() {
  ...
  useEffect(() => {
    ...
    // Helper to count remaining tasks for a given set
    const countRemaining = (taskList, day) =>
      taskList.filter(task => {
        if (task.status !== 'completed') return true;
        const completed = task.completedDate ? new Date(task.completedDate) : null;
        return !completed || completed > day;
      }).length;

    // Build 'all' records
    const allRecords = dateArray.map(day => ({
      date: day.toISOString().split('T')[0],
      tasksRemaining: countRemaining(tasks, day),
    }));

    // Build per-project records
    const projectRecords = projects.reduce((acc, proj) => {
      const projTasks = tasks.filter(t => t.projectId === proj.id);
      acc[proj.id] = dateArray.map(day => ({
        date: day.toISOString().split('T')[0],
        tasksRemaining: countRemaining(projTasks, day),
      }));
      return acc;
    }, {});

    setBurndownData({ all: allRecords, ...projectRecords });
  }, [tasks, projects]);
  ...
  const getBurndownChartData = () => {
    const data = burndownData[selectedBurndownProjectId] || [];
    return {
      labels: data.map(r => new Date(r.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })),
      datasets: [
        {
          label: 'Tasks Remaining',
          data: data.map(r => r.tasksRemaining),
